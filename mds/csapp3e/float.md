...
例如浮点数$3.14$和$31.4$，它们的“基础数值”相同（都是$314$），不同的只是小数点位置。理解这个之后就不难理解IEEE的浮点数标准，它也是由以上3部分组成。

IEEE的浮点数标准采用以下方式来定义一个浮点数

>$$V =  (-1)^s  \cdot M  \cdot 2^E$$ 

* $(-1)^s$表示正负号，当$s=0$时浮点数的值为正，$s=1$时为负。
* $M$表示基础数值，它是一个2进制的小数，值域为$[1,2-ε]$（规格化时）或$[0,1-ε]$（非规格化时）。
* $2^E$表示小数点位置（当$E>0$时候小数点向右移动反之向左移动）。 
...
* $s$：用1个bit表示，下文称$s$为“符号”。
* $M$：用$k$个bit表示，下文称$M$为“尾数”。
* $E$：用$n$个bit表示，下文称$s$为“阶码”。 

需要注意的是IEEE的标准并没有限定$k$和$n$的值，理论上它们可以为任意正整数（如果机器支持的话），同时也可以看出这个标准的优雅之处，不依赖特定平台，但是在任意平台上的计算规则完全一致。


$\begin{matrix} 1位 \\ \overbrace{s}\end{matrix}$
$\begin{matrix} k位 \\ \overbrace{e_1e_2.............e_k}\end{matrix}$
...
IEEE的浮点数标准分为3种情况，分别是规格化、非规格化、特殊值（又细分为两种情况），它们的区别如下所示
