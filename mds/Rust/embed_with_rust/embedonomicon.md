...
* ELF会被加载（烧录）到ROM（可以认为Rom就是PC中的硬盘）里面，但运行时却在RAM里面。（例如链接脚本里面就把`.bss`和`.data`分配到RAM中，**因此相关变量的地址是指向RAM的，即编译时候就认为这些数据/代码是在RAM的地址空间内运行**，而此时RAM里面的内容尚未被初始化（没有将ROM中ELF文件的`.bss`和`.data`复制到RAM中），直接读取的话会读到脏数据）
