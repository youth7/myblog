...
$$
$x$：输入空间的实例（一个$n$维向量）。   
$w$：权值向量（一个$n$维向量），$w$用来标记组成x的每个元素的权重。   
$b$：一个实数。

根据参考资料1，设$\vec{w} =(w_1,w_2...w_n )$是一向量，点$p = (p_1,p_2...p_n ), x = (x_1,x_2...x_n )$分别是空间中的一定点和动点，则$w \cdot \vec{px} = 0$描述了经过点$p$，并且以$\vec{w}$为法向量的点的集合（即超平面），代入具体坐标可得
$$
\begin{aligned}
w \cdot \vec{px} &= w_1(x_1-p_1)+w_2(x_2-p_2)...w_n(x_n-p_n) \\
...
误分类点到超平面的距离为 $\frac{-1}{||w||}y_i(w \cdot x_i + b)$（推理过程见参考资料），则总和（即损失函数）为：
$$L(w, b) = -\sum_{x_i∈M} =  y_i(w \cdot x_i + b)$$ 可以看出损失函数故意忽略了$\frac{-1}{||w||}$，在一些教材中并没有给出相关解释，从直观上看忽略的部分对函数的影响应该不大。
