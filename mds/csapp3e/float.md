...
**这是可以理解的，处于同一阶码的两个浮点数，其值的差是固定的，只与当前阶码的值有关。**
...
**说明产生进位的时候，其差可以用较小的数的阶码表达出来**。



比较①②可知，**规格化浮点数之间确实存在像整数那样的平滑过渡**。
...
这是可以理解的，因为非规格化的小数阶码全为0，因此不存在进位，所以任意两位相邻的值都是固定的。

比较③④可知，**非规格化浮点数之间确实存在像整数那样的平滑过渡，且差值是规格化差值公式中的特殊情况 （ $e_b=1$ ）**。
