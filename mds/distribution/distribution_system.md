...
在无主配置中，不存在故障转移（因为不需要主节点）。当故障节点恢复的时候，它的修复机制有：
...
Quorum机制是分布式系统重用来保证数据冗余和**最终一致性**的一种算法。注意😮😮😮😮，这里并**没有指出无主复制可以实现线性一致性**，**实际上Dynamo-Style并不能保证线性一致性**，见[DDIA的相关章节](https://github.com/Vonng/ddia/blob/master/ch9.md#什么使得系统线性一致)
