...
2. 为了实现SMR，我们需要**多副本日志（Replicated Log）**
...
1. 章节【一些疑问】中的`Q4`，如果一开始就带入错误的背景去理解Paxos，则认知会永远处于冲突之中。
...
4. Prepare阶段的目的：
   1. 找出已经被选中的值（如果有的话）
   2. block之前尚未完成的提议（如果有的话，如下面的情况3）
5. Accept阶段的目的：
   1. 通知acceptor去接受指定的值
...
注意，三种情况下写操作都不会丢失，要么使用之前的旧值（情况1和2），要么写入失败（情况3）。

>  但是情况1和2中，第2个客户端企图写入的值`Y`都失败了，不知道要怎么理解这种现象。但它符合算法的目标：并发时候一旦选定一个值后续就不再改动。
>
> 或许可以参考这篇回答：https://www.zhihu.com/question/19787937
>
> 
...
* 通过使用这个值来发送Accept请求，我们可以处理**上面的情况1和情况2，使系统最终达成共识**。
* 注意，**千万不要将这个情况与【分布式系统将一个值修改多次】这个关联起来**，当前场景是**并发时在集群中唯一确定一个值**。

---

`Q2.1`：如果必须使用Promise应答中的值，那么是否意味着当前Proposer的值丢失了？多个Proposer究竟需要怎么理解？

A：todo

---

`Q2.2`：提议编号究竟代表了什么，有什么作用？

A：代表了当前的提议是哪个Proposer提起出的，在并发情况下区分身份，并以此挑选最终获得写入权的那个请求者，作用**见`Q1`和情况3**
...
`Q5`：真的能实现线性一致性吗，单纯的quorum机制的不足怎么在这里就被弥补了？

A：单纯的quorum读写缺失原子性的问题被这样解决了：

* 一旦Acceptor接受了某个值，即使这个值暂时没被选中，后面的Proposal
...
2. 确定index的算法就是：**首个没有被chosen日志的下标（注意，经过这一步，index就确定了！）**
