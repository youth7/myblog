...
* stack上的数据：是静态的，指**静态大小**（编译期就能确定数据的大小），**静态生命周期**（与栈的生命周期一致）。其实这也不是绝对的，**例如`alloca()`就可以动态地在栈上分配内存**，C语言中的可变参也是分配在栈上的。但因为stack是较小且有限的，在上面动态分配空间容易导致溢出类问题。
* heap上的数据：是动态的，指**动态大小**（运行期间大小可能会改变），**动态生命周期**（可能会被多个栈引用）
...
1. 关于所有权的一些要点：
   ...
   * 要注意区分引用和借用的关系，官方文档中有：
     ...
     个人理解，*borrowing*，是rust编程模型提供的重点概念（和所有权、生命周期同一级别）。创建引用，则是 *borrowing* 这个概念在语言层面的具体实现。
   
   * Rust函数调用的时候**所有权会转移，例如`func(var)`并不会自动转为`func(&var)`，从而导致所有权转移**。



2. 通过所有权机制进行内存管理，解决了以下问题：
...
3. `Rc/Arc`提供了**共享所有权**（所有权规则的一个例外？），`Cell/RefCell`提供了**内部可变性**。这意味着除了`Rc/Arc`，其它标准库的数据对象都满足所有权规则。
...
貌似没什么特别的，关于API的话可以参考[这里](./unwrap_expect.md)



## 闭包

1. 闭包的大小跟捕获的参数相关，是存放在栈上的。有一个没解决的问题是闭包对应函数是怎么调用的，在编译阶段决定？还是通过指针传递？
