...
为了实现上述目标，lib.so中的代码（具体来说就是涉及地址访问的指令如地址跳转）必须做一些调整。因为lib.so是被多个进程共享的，所以可能被映射到不同的地址空间中。如果lib.so中的地址访问用到了**绝对地址**，则它无法被多个进程共享。

例如，下图是Linux在X86_64架构上的进程内存布局，假设程序p1启动时加载lib.so并对其中某个函数`func`的地址重定位为0xFFFF（**注意：这意味着lib.so在物理内存中的跟地址相关的内容被修改了，这会影响到稍后启动的p2**），0xFFFF对p1来说是合法的。然而，如果另外一个也依赖lib.so的程序p2在稍后启动，并且它也调用了函数`func`，因为这个函数已经在p1启动时被重定位过了，所以此时它的地址是0xFFFF。但在p2内，0xFFFF可能已经被p2自己使用了，它不再是一个合法的地址空间。
