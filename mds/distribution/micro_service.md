...
服务的注册和发现可以参考腾讯技术功能的：[深入了解服务注册与发现](https://zhuanlan.zhihu.com/p/161277955)，总的来说可以分为两种方式：

* 应用层服务发现

  **客户端发现 + 服务自注册**（弊端是不同类型的客户端需要自己编写相同逻辑的代码，例如java和go客户端都要编写类似的代码去访问同一个后端服务）

* 平台服务层发现模式

  **服务端发现 + 第三方（平台）注册**（弊端是依赖于部署平台，例如k8s）
