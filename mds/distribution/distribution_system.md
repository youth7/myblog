...
> 注意**法定人数不一定是大多数**，法定人数的核心是有**重叠**，这样即使不是大多数的情况下依然能够返回最新数据
...
1. 使用宽松的法定人数（个人认为使用宽松的话就背离了该算法的核心了，虽然这个算法确实允许使用宽松）
2. 并发写入时的数据丢失
...
6. 即使一切工作正常，有时也会不幸地出现关于 **时序（timing）** 的边缘情况，我们将在 “[线性一致性和法定人数](https://github.com/Vonng/ddia/blob/master/ch9.md#线性一致性和法定人数)” 中看到这点。



解读：

1. 宽松的限制必然增加返回陈旧数据的几率，这可以理解
2. todo
...
Quorum机制是分布式系统重用来保证数据冗余和**最终一致性**的一种算法。注意😮😮😮😮，这里并**没有指出无主复制可以实现线性一致性**，**实际上Dynamo-Style并不能保证线性一致性**，如下图所示，详细参考[DDIA的相关章节](https://github.com/Vonng/ddia/blob/master/ch9.md#什么使得系统线性一致)
...
2. 多主复制：非线性一致
3. 无主复制：**也许不是线性一致的**，但通过牺牲性能，可以使 Dynamo 风格的法定人数线性化：读取者必须在将结果返回给应用之前，同步执行读修复，并且写入者必须在发送写入之前，读取法定数量节点的最新状态（这样就可以了？感觉在极端情况下，只有读取整个集群全部副本数据才能实现线性一致）。
...
*  [第九章：一致性与共识](https://github.com/Vonng/ddia/blob/master/ch9.md#%E7%AC%AC%E4%B9%9D%E7%AB%A0%E4%B8%80%E8%87%B4%E6%80%A7%E4%B8%8E%E5%85%B1%E8%AF%86)
...
和线性一致性的区别在于**没有全局时钟的限制**，因此：

* 顺序一致性：要求**系统的全体操作**在**单个客户端**的角度来看是有序的
* 线性一致性：要求**系统的全体操作**对**所有客户端**都是有序的
