...
注意在链接时候，目标文件的顺序是非常敏感的，不能贪图方便直接写`*.o`，因为：

1. QEMU会固定在内存中`0x80000000`开始执行用户代码，并且QEMU会将ELF文件中地址为`0x80000000`的代码加载到这里。
2. 而链接时，我们通过参数`-Ttext=0x80000000`将`.text`的地址指定为`0x80000000`。
3. 因此我们在编译链接时，需要将`start.s`的中的`_start`的代码固定到`.text`的开始，即`0x80000000`
4. 在没有额外的链接脚本参与控制顺序的情况下，我们为了实现步骤3，只能手动指定.o文件的顺序，即让`start.o`放在第一位。
5. 而链接时用`*.o`的话，按照linux下的默认规则，会将`kernel.o`放在首位，从而出错
