...
2. `push_context`会消费1中的对象（move进去），然后生成一个`* mut TrapContext`类型的`cx_ptr`（指针）指向1中的对象（**`cx_ptr`此时正指向内核栈，这一点非常重要**）
3. `*cx_ptr = cx`，相当于将传递进来的`cx`数据复制到内核栈
4. 2中的`cx_ptr`会传递给`__restore`，如1中所说，`cx_ptr`正指向内核栈，因此`a0`的值就是内核栈的地址了（注意只关注`cx_ptr`指向哪里，而不是`cx_ptr`本身在哪里），因此`__restore`中的`mv sp, a0`就得到了解释：
...
具体到代码就是通过*build.rs*生自动成了*link_app.S*，后者把几个用户程序都链接到OS的`.data`节中并暴露出相关符号，最后由`AppManager`实现加载。

之所以用build.rs是为了方便当增加用户程序之后OS能够自动生成链接文件无需手动修改*link_app.S*
