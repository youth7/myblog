...
## 关于caller save和callee save：

假设在一次函数调用中，我们指定m个寄存器是caller save，剩下的n个寄存器是callee save，那么每次调用都必须保存m+n个寄存器。这意味着保存上下文（将寄存器内容写入内存）的时候，**读写内存的数量是不变的**，从这个角度来看，似乎规定谁保存都没有意义。

只有在编译生成机器码时候，根据当前函数的寄存器使用情况进行优化，caller save和callee save才有意义。即对caller来说，编译器会回溯当前寄存器使用情况，**把m个寄存器中真正用到的那些进行保存**（对callee来说也如此）。这样的话才能最大限度减少内存读写。

但上述只有在开启了编译器优化才能实现，否则编译器还是会固定保存寄存器。
